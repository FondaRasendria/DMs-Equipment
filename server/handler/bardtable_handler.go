...
func BardTableHandlerGetAll(ctx *fiber.Ctx) error {
	var models []model.BardTable
...
func BardTableHandlerCreate(ctx *fiber.Ctx) error {
	request := new(request.BardTableCreateRequest)
...
func BardTableHandlerGetById(ctx *fiber.Ctx) error {
	id := ctx.Params("id")

	var model model.BardTable
	...
	response := response.BardTableResponse{
		Id:           model.Id,
...
func BardTableHandlerUpdate(ctx *fiber.Ctx) error {
	request := new(request.BardTableUpdateRequest)
	if err := ctx.BodyParser(request); err != nil {
		return ctx.Status(400).JSON(fiber.Map{
			"message": "bad request",
		})
...
func BardTableHandlerDelete(ctx *fiber.Ctx) error {
	id := ctx.Params("id")
	var model model.BardTable
