...
func ClericTableHandlerGetAll(ctx *fiber.Ctx) error {
	var models []model.ClericTable
...
func ClericTableHandlerCreate(ctx *fiber.Ctx) error {
	request := new(request.ClericTableCreateRequest)
...
func ClericTableHandlerGetById(ctx *fiber.Ctx) error {
	id := ctx.Params("id")

	var model model.ClericTable
	...
	response := response.ClericTableResponse{
		Id:           model.Id,
...
func ClericTableHandlerUpdate(ctx *fiber.Ctx) error {
	request := new(request.ClericTableUpdateRequest)
	if err := ctx.BodyParser(request); err != nil {
		return ctx.Status(400).JSON(fiber.Map{
			"message": "bad request",
		})
...
func ClericTableHandlerDelete(ctx *fiber.Ctx) error {
	id := ctx.Params("id")
	var model model.ClericTable
