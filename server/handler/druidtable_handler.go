...
func DruidTableHandlerGetAll(ctx *fiber.Ctx) error {
	var models []model.DruidTable
...
func DruidTableHandlerCreate(ctx *fiber.Ctx) error {
	request := new(request.DruidTableCreateRequest)
...
func DruidTableHandlerGetById(ctx *fiber.Ctx) error {
	id := ctx.Params("id")

	var model model.DruidTable
	...
	response := response.DruidTableResponse{
		Id:           model.Id,
...
func DruidTableHandlerUpdate(ctx *fiber.Ctx) error {
	request := new(request.DruidTableUpdateRequest)
	if err := ctx.BodyParser(request); err != nil {
		return ctx.Status(400).JSON(fiber.Map{
			"message": "bad request",
		})
...
func DruidTableHandlerDelete(ctx *fiber.Ctx) error {
	id := ctx.Params("id")
	var model model.DruidTable
